...
const CREATE_PROMPTS: Record<string, string> = {
  ...
  dashboard:
    'Собери дашборд из моих данных. Сейчас загружу файл с таблицей (Excel или CSV) или пришлю ссылку на открытую Google-таблицу.',
};
...
const BOT_PROMPTS: Record<string, string> = {
  ...
  leads:
    'Хочу Telegram-бота, который собирает обращения и контакты клиентов в диалоге. Проведи по шагам подключения (@BotFather → токен → канал агента) и помоги настроить, что спрашивать у клиента. Что нужно собирать: ',
  shop: 'Хочу Telegram-бота-консультанта по моим товарам или услугам — чтобы он рассказывал о них, подбирал подходящее и отвечал на вопросы клиентов. Проведи по шагам подключения (@BotFather → токен → канал агента). Что я предлагаю: ',
};
