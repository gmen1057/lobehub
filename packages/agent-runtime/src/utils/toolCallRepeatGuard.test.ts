...
const createToolCall = (
  ...
  overrides?: Partial<ChatToolPayload>,
): ChatToolPayload => ({
  apiName: 'inject',
...
describe('toolCallRepeatGuard', () => {
  ...
  it('resets the consecutive count when a different tool is called', () => {
    let guard: ReturnType<typeof updateToolCallRepeatGuard> | undefined;

    for (let index = 0; index < 4; index++) {
      guard = updateToolCallRepeatGuard(guard, [createToolCall('{"keys":["github"]}')]);
    }
    expect(hasRepeatedToolCall(guard)).toBe(false);

    const resetGuard = updateToolCallRepeatGuard(guard, [
      createToolCall('{}', { apiName: 'search', identifier: 'lobe-web-browsing' }),
    ]);

    expect(hasRepeatedToolCall(resetGuard)).toBe(false);
    expect(Object.values(resetGuard.counts)).toEqual([1]);
  });
});
