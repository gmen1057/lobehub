...
1. Analyze the data ONLY through executeCode (pandas), never by hand. For a link (Google Sheet or any URL) — download the CSV inside the sandbox, it has internet access. For an uploaded file — the chat message already carries its full parsed content plus a working url; do NOT hand-read or hand-tally that pasted table, it is the entire dataset rather than a summary and manual counting is slow and unreliable, so re-download the file inside the sandbox via that url and work through pandas instead. Compute the aggregates you need and record each column's name and type (string/number/date/bool).
